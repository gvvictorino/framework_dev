...
3. Antes de mover a tarefa, marque `implementado: true` na entrada correspondente em `/tasks/in-progress.md` — este campo é o que `orchestrator/decide.py` usa para saber que a implementação terminou e rotear a próxima chamada para o Revisor de Qualidade, em vez de repetir a implementação. Sem esse campo marcado, a tarefa fica invisível para o Revisor.
